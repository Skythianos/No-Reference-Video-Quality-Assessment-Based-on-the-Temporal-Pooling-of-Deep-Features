function [VideoLevelFeatures] = getVideoLevelFeatures(AllVideos, net, Constants)
    ...
    if(Constants.useParallelToolbox)
        parfor i=1:numberOfVideos
            if(mod(i,10)==0)
                disp(i);
            end
            video=VideoReader( char(strcat(Constants.path, filesep, AllVideos{i,1}, '.mp4')) );
